...
exports.lambdaHandler = async (event, context, callback) => {
   ...
   // ready to go for CORS. To make this a completed HTTP response, you only need to add a statusCode and a body.
    ...
    try {
       ...
       var sql2 = "select * from info.Inventory join info.Locations on info.Locations.sku = info.Inventory.sku where storeID =? and aisle =? and shelf =?";
        ...
        //     let it = e[i]
         ...
         let quick_result = await query(pool, sql2 , [storeID, aisle, shelf] ) 
            //   result.forEach((row) => {
            //       console.log(row.storeID, row.sku, row.aisle, row.shelf);
             if(quick_result.length === 0) {
              //
               console.log('result:' + "There is no Item on aisle and shelf in this store location")
