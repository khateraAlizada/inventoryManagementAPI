...
// }
  ...
  function compute_dist(custLat, custLong,storeLat, storeLong) {
    ...
    try {
     ...
     //  var sqllatLong = "SELECT latitude, longitude, idstore FROM info.Store";
      ...
      var sqllatLong2 = "SELECT latitude, longitude, idstore, Items.sku, quantity, Items.name, Items.description FROM ((info.Store join info.Inventory on info.Inventory.storeID=info.Store.idstore) join Items on info.Inventory.sku = info.Items.sku) where info.Items.name like ? and info.Items.description like ?";
      var sqlsku = "SELECT latitude, longitude, idstore, Items.sku, quantity, Items.name, Items.description FROM ((info.Store join info.Inventory on info.Inventory.storeID=info.Store.idstore) join Items on info.Inventory.sku = info.Items.sku) where info.Inventory.sku=? ";
  ...
  let values = [];
   ...
   let skuSearch = await query(pool, sqlsku, [sku])
   if (skuSearch.length > 0){
       //let values = []
        for (let r in skuSearch) {
            let d = compute_dist(custLat, custLong, skuSearch[r].latitude, skuSearch[r].longitude)
            skuSearch[r].distance = d
            console.log("r:" + JSON.stringify(skuSearch[r]))
            values.push(skuSearch[r])
        }
        console.log("rows.distance " + values.distance)
       ...
       let rows = await query(pool, sqllatLong2, [["%"+[name]+"%"], ["%"+[description]+"%"]])
       if (rows.length>0){
      //  let rows = await query(pool, sqllatLong2,[[sku],[[name]+'%'] ] )
        console.log("[name] +%:" + name + description);
        
        console.log("all stores location" + JSON.stringify(rows))
       // let values = []
        for (let r in rows) {
