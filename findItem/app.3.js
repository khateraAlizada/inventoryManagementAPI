...
// }
  ...
  function compute_dist(custLat, custLong,storeLat, storeLong) {
    ...
    try {
       ...
       var sqllatLong2 = "SELECT latitude, longitude, idstore, sku, quantity FROM info.Store join info.Inventory on info.Inventory.storeID=info.Store.idstore where info.Inventory.sku =?";
        
       
         let success = true
     
  // use query to get ALL STORE rows
        let rows = await query(pool, sqllatLong2, [sku] )
