...
// );
    ...
    // const quick_result = await InsertTuple(info.sku,info.name,info.description,info.price);
       ...
       var sql6= "Select maxQ from info.Items where sku=?";
        ...
        for (let i = 0; i < sh.length; i++) {
            ...
            //a row doesnot exist
             if(result.length === 0) {
                 ...
                 let checkMaxQ = await query(pool, sql6 , [ it.sku]);
                    console.log('MaxQ:' + JSON.stringify(checkMaxQ));
                    
                    // let os = quantity - maxQ
                    // if os <= 0
                    // then normal insert
                     
                    // else insert os in overstock 
                    // // take care of updation as well
                    // and maxQ in inventory
                    
                //     let currentQuantity = await query(pool, sql4,[it.sku]) // get the quantity for that SKU
                // if (currentQuantity.length == 0) {
                //     console.log("sku does not exist in inventory " + it.sku);
                    
                // } else {
                //     let actual = parseInt(currentQuantity[0].quantity)
                //     console.log('currentQuantity sql4:' + actual +"," + storeID)
                //     let checkMaxQ = await query(pool, sql6 , [ it.sku]);
                //     console.log('MaxQ:' + JSON.stringify(checkMaxQ))
                    
                    let QI = await query(pool, sql4,[it.sku]) // get the quantity for that SKU
                    if (QI.length == 0) {
                    console.log("This sku does not exist in Inventory table " + it.sku);
                    let actualCQI = parseInt(CQI[0].quantity);
                    checkMaxQ = JSON.stringify(checkMaxQ);
                    
                    
                    console.log("CQI:"+ actualCQI);
                    
                    console.log("checkMax:"+ checkMaxQ);
                    let os = actualCQI - checkMaxQ;
                    ...
                    console.log("os  amount is" + os);
                    
                    if (os <= 0)
                    {
                       let insertTupleNormal= await query(pool, sql2 , [ it.sku, JSON.stringify(CQI)]);
                        // resolve because we have result(s) from the query. it may be an empty rowset or contain multiple values
                       console.log('resultInsertQuantityNormal:' + insertTupleNormal.length);
                    }
                    
                    else
                    
                    {
                      let insertTupleOS= await query(pool, sql7 , [ storeID, parseInt(os), it.sku]);
                      
                      let insertTuple= await query(pool, sql2 , [ storeID, it.sku , checkMaxQ]);
            ...
            // when a row exists
                ...
                if (currentQuantity.length == 0) {
                    ...
                    // let temp = actual + updated quantity
                    // let os = temp - maxQ
                    // if os <= 0
                    ...
                    let updateQuantity = await query(pool, sql3, [parseInt(it.quantity) + actual, it.sku , storeID]) ; //add to the  prev quantity
