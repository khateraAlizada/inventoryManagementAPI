...
// );
    ...
    let ProcessShipment = (shipment) => {
        
       return new Promise((resolve, reject) => {
              pool.query("INSERT INTO info.Shipment (sku, quantity) VALUES (?,?)",[shipment.map(item => [item.sku, item.quantity])], (error, rows) => {
    ...
    try {
       ...
       // const quick_result = await InsertTuple(sku,name,description,price);
        ...
        const quick_result = await ProcessShipment(shipment);
