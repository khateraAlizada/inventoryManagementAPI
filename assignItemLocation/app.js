...
exports.lambdaHandler = async (event, context, callback) => {
   ...
   // ready to go for CORS. To make this a completed HTTP response, you only need to add a statusCode and a body.
    ...
    let AssignItemLocation = (idlocations,sku,aisle,shelf) => {
       
       return new Promise((resolve, reject) => {
              pool.query("INSERT INTO info.Locations ( idlocations,sku, aisle, shelf) VALUES(?, ?, ? ,? )", [idlocations,sku,aisle, shelf], (error, rows) => {
    ...
    try {
       ...
       // const quick_result = await InsertTuple(sku,name,description,price);
        const quick_result = await AssignItemLocation (info.idlocations,info.sku,info.aisle,info.shelf);
