...
exports.lambdaHandler = async (event, context, callback) => {
   ...
   // ready to go for CORS. To make this a completed HTTP response, you only need to add a statusCode and a body.
    ...
    let InsertTuple = (sku,name,description,price, maxQ) => {
       
       return new Promise((resolve, reject) => {
              pool.query("INSERT INTO info.Items ( sku, name , description, price,maxQ) VALUES(?, ?, ? , ?,? )", [sku,name,description,price,maxQ], (error, rows) => {
    ...
    try {
       ...
       // const quick_result = await InsertTuple(sku,name,description,price);
        const quick_result = await InsertTuple(info.sku,info.name,info.description,info.price,info.maxQ);
