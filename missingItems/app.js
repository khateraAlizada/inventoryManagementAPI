...
exports.lambdaHandler = async (event, context, callback) => {
   ...
   // ready to go for CORS. To make this a completed HTTP response, you only need to add a statusCode and a body.
    ...
    var storeID = e.storeID;
     var managerUserName = e.managerUserName;
    var managerPassword = e.managerPassword; 
    ...
    try {
        ...
        var sqlMissingItems = "select * from info.Items where sku not in (select distinct sku from info.Inventory where storeID=?) ";
       var sqlAut ="select managerUserName, managerPassword from info.Store where idstore=?";
         ...
         let quick_result = await query(pool, sqlMissingItems,[storeID]);
